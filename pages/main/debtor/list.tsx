...
export async function getServerSideProps(context: any) {
  ...
  try {
    const result = await axios.get(
      CONFIG.base_url_api +
        `/debtor/list?search=${search || ""}&page=${
          +page > 0 ? +page : +page - 1
        }&size=${+size || 10}`,
      {
        headers: {
    ...
    const users = await axios.get(
      CONFIG.base_url_api + `/user/list?from=admin`,
      {
        headers: {
          "bearer-token": "kaltengventura2023",
        },
...
export default function List({ table, users }: any) {
  ...
  const USERS: any = users?.map((v: any) => {
    return { ...v, value: v?.id, label: v?.name?.toUpperCase() };
  });
    ...
    },
    {
      ...
      selector: (row: Debtor) => (
        <>
          ...
          {row?.status == "approved" &&
            users?.find((v: any) => v?.id == row?.user_id)?.from == "admin" && (
              <button
                onClick={() => {
              ...
              onClick={() => {
                ...
                localStorage.setItem(
                  ...
                  JSON.stringify({
                    ...
                    user_from: users?.find((v: any) => v?.id == row?.user_id)
                      ?.from,
                  })
                );
              }}
              ...
              <div>
                ...
                <form onSubmit={handleVerification}>
                  ...
                  <p className="text-center">
                    ...
                    {modal.key == "approved"
                      ? `menyetujui ${modal.data.name}`
                      : `menolak ${modal.data.name}`}
                    ?
                  </p>
                  <div className="my-4">
              ...
              <div>
                ...
                <form onSubmit={save}>
                  ...
                  <Select
                    label="Pengguna"
                    required
                    options={[{ value: "", label: "Pilih Pengguna" }, ...USERS]}
                    defaultValue={modal?.data?.user_id || ""}
                    name="user_id"
                  />
                  <Input
                    label="Nama / Nama Usaha"
                    placeholder="Masukkan Nama / Nama Usaha"
                    name="name"
                    required
                  ...
                  <div className="flex md:flex-row flex-col gap-2">
                    <Input
                      ...
                      name="field_type"
                      required
                      defaultValue={modal?.data?.field_type || ""}
                    />
                    <Input
                      ...
                      name="place_status"
                      required
                      defaultValue={modal?.data?.place_status || ""}
                    />
                  </div>
                  <Input
                    ...
                    name="mother_name"
                    required
                    defaultValue={modal?.data?.mother_name || ""}
                  />
                  <div className="mt-2">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          defaultChecked={modal?.data?.other_loan == "1"}
                          name="other_loan"
                          value={"1"}
                        />
                        <span>Ya</span>
                  ...
                  {loans && (
                    <Input
                      ...
                      name="other_loan_name"
                      required
                      defaultValue={modal?.data?.other_loan_name || ""}
                    />
                  )}
